...
export const ProjectTableScreen = observer(function ProjectTableScreen({route}: any) {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      getContent().then(async () => {
        let arrangedBomItems = await projectContentStore.specificBomItemSet(route.params.requestId).map(item => {
          return [
            item.node.code || '-',
